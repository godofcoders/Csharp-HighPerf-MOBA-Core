...
{
    public sealed class BombLeapAbilityLogic : IAbilityLogic
    {
        ...
        private readonly BombLeapAbilityDefinition _definition;
        private readonly List<ISpatialEntity> _targetBuffer = new List<ISpatialEntity>(24);

        public BombLeapAbilityLogic(BombLeapAbilityDefinition definition)
        {
        ...
        public AbilityExecutionResult Execute(IAbilityUser user, AbilityExecutionContext context)
        {
            ...
            owner.RunTimedBurst(ResolveLandingRoutine(owner, landing, travelDuration));
            owner.RunTimedBurst(ResolveBombRoutine(owner, context, origin, forward));

            AbilityExecutionResult result = AbilityExecutionResult.Succeeded(
                context.AbilityDefinition,
                context.SlotType);
            result.AppliedAreaEffect = true;
            result.TargetsAffected = Mathf.Max(1, _definition.BombCount);
        ...
        private IEnumerator ResolveBombRoutine(
            BrawlerController owner,
            ...
            Vector3 forward)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                int affected = ApplyBombDamage(owner, context, bombPosition);
        ...
        private int ApplyBombDamage(
            BrawlerController owner,
            AbilityExecutionContext context,
            Vector3 bombPosition)
        {
            if (SimulationClock.Grid == null || _definition.BombDamage <= 0f)
                return 0;

            IDamageService damageService = ServiceProvider.Get<IDamageService>();
            float radius = Mathf.Max(0.05f, _definition.BombRadius);
            int targetsAffected = 0;

            _targetBuffer.Clear();
            SimulationClock.Grid.GetEntitiesInRadiusNonAlloc(bombPosition, radius + 1f, _targetBuffer);
            ...
            for (int i = 0; i < _targetBuffer.Count; i++)
            {
                ...
                Vector3 toTarget = target.Position - bombPosition;
                toTarget.y = 0f;
        ...
        private Vector3 ResolveBombPosition(
            Vector3 origin,
            Vector3 forward,
            int index,
            int count)
        {
            float radius = Mathf.Max(0f, _definition.BombSpreadRadius);
            if (radius <= 0.001f || count <= 1)
                return origin;
            ...
            right.Normalize();
            Vector3 forwardFlat = forward.sqrMagnitude > 0.001f ? forward.normalized : Vector3.forward;

            float angle = (Mathf.PI * 2f * index / count) + (Mathf.PI * 0.25f);
            Vector3 offset =
                (right * Mathf.Cos(angle) + forwardFlat * Mathf.Sin(angle)) * radius;
