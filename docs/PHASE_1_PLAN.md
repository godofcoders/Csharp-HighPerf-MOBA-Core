...
| 11–12 | Gem Grab: objective scaffolding | ✅ done (Session 7) | Carrier substate + Gem entity + GemSpawner + GemGrabMode coordinator with death-drop. Win-condition is a 16s hold timer (not 60s as originally noted — corrected to Brawl Stars rule). | Match-lifecycle state machine; objective entities |
| 13–14 | Gem Grab: objective-aware AI + win condition | ✅ done (Session 7) | 3 AIUtilityScorer boosts: retreat-by-carried-gems, approach-when-behind, super-on-fat-carrier. Carrier-protection peel + gem-hunger steering deferred until playtest shows them needed. Win timer integrated into MatchManager via AddScore on expiry. | How objective AI differs from combat AI |
